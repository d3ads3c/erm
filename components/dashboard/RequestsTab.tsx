...
export default function RequestTab() {
  ...
  if (permission) {
    const AcceptImprest = async (ID: number) => {
      await fetch("/api/imprest/accept", {
        method: "POST",
    ...
    const RejectImprest = async (ID: number) => {
      await fetch("/api/imprest/reject", {
        method: "POST",
        body: JSON.stringify({ ID: ID })
      }).then((res) => res.json())
        .then((data) => {
              ...
              imprest.Status == "pending" && (
                <div className="w-full" key={imprest.ID}>
                  <div className="border rounded-2xl p-3 space-y-3">
                    ...
                    <div>
                      {imprest.Status == "pending" && (
                        imprest.Permission.includes("13") && imprest.Ceo == "pending" ? (
                          <div className="flex items-center justify-center gap-5 mt-1">
                            <div className="w-1/2">
                              <button onClick={() => AcceptImprest(imprest.ID)} className="text-emerald-400 text-center w-full border-emerald-400 rounded-xl border py-2 hover:bg-emerald-400 hover:text-white duration-150">
                                <p>تایید</p>
                              </button>
                            </div>
                            <div className="w-1/2">
                        ...
                        ) : imprest.Permission.includes("4") && (imprest.Ceo == "0" || imprest.Ceo == 'accepted') && (
                          <div className="flex items-center justify-center gap-5 mt-1">
                            <div className="w-1/2">
                              <button onClick={() => AcceptImprest(imprest.ID)} className="text-emerald-400 text-center w-full border-emerald-400 rounded-xl border py-2 hover:bg-emerald-400 hover:text-white duration-150">
                                <p>تایید</p>
                            ...
                            <div className="w-1/2">
                              <button onClick={()=>RejectImprest(imprest.ID)} className="text-red-500 text-center w-full border-red-500 rounded-xl border py-2 hover:bg-red-500 hover:text-white duration-150">
                                <p>رد</p>
                              </button>
                            </div>
                          </div>
